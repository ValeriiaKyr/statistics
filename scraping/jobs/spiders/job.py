...
class JobSpider(scrapy.Spider):
    ...
    start_urls = [
        "https://www.pracuj.pl/praca/python;kw?utm_source=google&utm_medium=cpc&utm_campaign=dsa_miasta_an&campaignid=20387307111&adgroupid=157252921568&gad_source=1&gclid=CjwKCAiAxKy5BhBbEiwAYiW--xCq-CDIBRyZwDEC0tZeGD6dr2OlbSgwcJd2BAw3hvwtve20L1uBnBoCZzQQAvD_BwE"
    ]
    ...
    def parse(self, response: Response, **kwargs) -> dict:
        first = response.css(
            "div.listing_ohw4t83 div.type--positioned div.tiles_cobg3mp a.tiles_cnb3rfy::attr(href)"
        ).getall()
        next = response.css(
            "div[data-test='section-offers'] div.tiles_b18pwp01 a.tiles_cnb3rfy::attr(href)"
        ).getall()
        ...
        num_pages = response.css(
            "span[data-test='top-pagination-max-page-number']::text"
        ).get()
    ...
    def _parse_additional_info(
        ...
        info = {
            "title": detail_response.css("h1.ow09k7q::text").get(),
            "company": detail_response.css("h2.cp9trhu::text").get(),
            "expected": [
                li.css("::text").get()
                for li in detail_response.css(
                    "div[data-test='section-technologies-expected'] ul.cku9rky li.catru5k"
                )
            ],
            "optional knowledge": [
                li.css("::text").get()
                for li in detail_response.css("ul.cku9rky li.catru5k")
            ],
            "link": detail_url,
            "project": detail_response.css(
                "div.cu3huna ul.c1qnwb5g li.t6laip8::text"
            ).getall(),
